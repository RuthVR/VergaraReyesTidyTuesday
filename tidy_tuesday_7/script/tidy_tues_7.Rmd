...
netflix_2021_data <- clean_netflix %>%
  filter(year_added != 2008, year_added != 2013,
         ...
         year_added != 2018, year_added != 2019)%>%
...
# Making the Plot
```{r}
ggplot(data = netflix_2021_data,
         aes(y = 0,
             x = 0))+                                ## Making the beginning of the plots 
  facet_wrap(~listed_in, ncol = 5 )+                 ## going to facet wrap by the genre of the movie
  geom_image(data = images, 
             aes(x=0.056, y=0.056, image = image), 
             size = 1.4, by="width")+                ## Determining the width of the background image (cartoon tv)
  annotate("rect", 
           xmin = -0.38, ymin = -0.27, xmax = 0.37, ymax = 0.28, 
           fill = "black", color = "black")+         ## Making the background of the plot black and determining the diameters
  geom_jitter(aes(y = -0.04, 
                  size = duration,
                  color = rating),                   ## making the colors based on the ratings of the films. 
              size = 1, width = 0.35, height = 0.2,  ## determining the size of the points 
  ...
  stat_summary(aes(color = rating, 
               ...
               fun = "mean", 
               geom = "text", 
               size = 3, family = "Bebas Neue", 
               color = "white", 
               lineheight = 0.8)+                    ## Editing the writing within the plot
  theme_ft_rc()+                                     ## Used to make sure nothing is being cut of in the plots
  ...
  theme(legend.position = "none", 
        ...
        plot.title = element_markdown(hjust = 0.5, family = "Bebas Neue", 
                                      size = 30, color = "white"))+       ## Editing theme elements of the plot
...
ggsave(here("tidy_tuesday_7", "output", "Netflix_plot.png"),
       width = 10, height = 7)                       ## saving the plot as an  
